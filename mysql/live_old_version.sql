-- =============================================
-- 自动生成的数据库结构文件
-- 生成时间: 2026-05-21T05:53:46.147Z
-- 数据库: live_new
-- 来源: SHOW CREATE TABLE（仅结构，不含数据）
-- 说明: 请勿直接在已有数据的在线库执行 DROP/CREATE 模板
-- =============================================
...
CREATE TABLE IF NOT EXISTS `account` (
  ...
  KEY `idx_transfer_group` (`transfer_group_id`)
...
CREATE TABLE IF NOT EXISTS `bus_recurring` (
  ...
  `category_id` varchar(50) DEFAULT NULL COMMENT '关联分类',
  `account_id` varchar(50) DEFAULT NULL COMMENT '默认扣款账户/卡片',
  ...
  `remind_days` int(11) DEFAULT '0' COMMENT '提前提醒天数',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注',
  `month_records` json DEFAULT NULL COMMENT '月度状态记录，键为YYYY-MM',
...
CREATE TABLE IF NOT EXISTS `card_repay` (
  ...
  `repay_card_id` varchar(50) NOT NULL COMMENT '还款来源的的id',
...
CREATE TABLE IF NOT EXISTS `moment` (
  ...
  `img_url` varchar(10000) DEFAULT NULL COMMENT '图片',
  `mood` varchar(50) DEFAULT NULL COMMENT '心情',
  `location` varchar(255) DEFAULT NULL COMMENT '位置',
  `visible_type` varchar(255) DEFAULT '{\"vt\":0,\"vs\":0,\"pw\":0}' COMMENT '可见性(JSON): vt可见类型/vs批次号/pw密码',
