...
CREATE TABLE IF NOT EXISTS `account` (
  ...
  `amount` decimal(14,4) NOT NULL COMMENT '金额（原币种，外币按真实精度存储）',
...
CREATE TABLE IF NOT EXISTS `bus_recurring` (
  ...
  `account_id` varchar(1000) DEFAULT NULL COMMENT '关联卡片/分期信息JSON',
  `cycle` varchar(20) DEFAULT NULL COMMENT '周期: month/year/week',
  `day_of_cycle` int(11) DEFAULT NULL COMMENT '周期内的哪一天',
  `month_of_cycle` int(11) DEFAULT NULL COMMENT 'year周期对应的月份(1-12)',
  `next_date` varchar(20) DEFAULT NULL COMMENT '下次触发日期',
  `remind_days` int(11) DEFAULT '0',
  `remark` varchar(255) DEFAULT NULL,
  `month_records` json DEFAULT NULL,
  `end_date` varchar(20) DEFAULT NULL COMMENT '到期日期(YYYY-MM-DD)，到期后停止生成',
  `repeat_count` int(11) DEFAULT NULL COMMENT '总期数(如12期分期)，完成后期数后自动停用',
  `notify_channel` varchar(50) DEFAULT 'app' COMMENT '推送渠道: app/sms/email/none',
...
CREATE TABLE IF NOT EXISTS `card_bill` (
  ...
  `used_limit` decimal(14,4) NOT NULL COMMENT '已用额度',
  ...
  `bill_amount` decimal(14,4) DEFAULT NULL COMMENT '本期账单',
  `min_repay` decimal(12,2) DEFAULT NULL COMMENT '最低还款',
  `repaid` decimal(12,2) DEFAULT NULL COMMENT '已还金额',
  `need_repay` decimal(14,4) DEFAULT NULL COMMENT '待还金额',
...
CREATE TABLE IF NOT EXISTS `moment` (
  ...
  `visible_type` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT '{\"vt\":0,\"vs\":0,\"pw\":0}' COMMENT '可见性(JSON): vt可见类型/vs批次号/pw密码',
