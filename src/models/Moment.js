...
class Moment {
   ...
   */
  static async create({ userId, parentId, content, imgUrl, mood, location, visibleType = 0 }) {
    const id = nanoid();
    ...
    await db.execute(sql, [
      id, userId, finalParentId, content || "", imgUrl, mood, location, visibleType, now, now,
    ]);
    return { id };
  }
  ...
  static async findTodayMain(userId) {
    ...
    const sql = `
      ...
      WHERE user_id = ? 
        ...
        AND is_deleted = 0 
      LIMIT 1`;
    const [rows] = await db.execute(sql, [userId, start.getTime().toString(), end.getTime().toString()]);
    return rows[0] || null;
  }
  ...
  static async update(id, userId, updates) {
    ...
    if (updates.imgUrl !== undefined) { fields.push("img_url = ?"); values.push(updates.imgUrl); }
    ...
    if (updates.visibleType !== undefined) { fields.push("visible_type = ?"); values.push(updates.visibleType); }
    ...
    const sql = `UPDATE moment SET ${fields.join(", ")} WHERE id = ? AND user_id = ? AND is_deleted = 0`;
    const [result] = await db.execute(sql, values);

    if (result.affectedRows === 0) return { status: 404, message: "动态不存在或无权修改" };
  ...
  static async delete(id, userId) {
    const sql = `UPDATE moment SET is_deleted = 1, update_time = ? WHERE id = ? AND user_id = ? AND is_deleted = 0`;
    const [result] = await db.execute(sql, [Date.now().toString(), id, userId]);

    if (result.affectedRows === 0) return { status: 404, message: "动态不存在或无权删除" };
