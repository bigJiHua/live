  ...
  <div class="page-device-info">
    ...
    <div class="info-card">
      ...
      <van-cell-group inset>
        <van-cell title="浏览器" :value="uaInfo.browser.name + ' ' + (uaInfo.browser.version || '')" />
        <van-cell title="操作系统" :value="uaInfo.os.name + ' ' + (uaInfo.os.version || '')" />
        ...
        <van-cell title="渲染引擎" :value="uaInfo.engine.name + ' ' + (uaInfo.engine.version || '')" />
      </van-cell-group>
    </div>
    ...
    <div class="info-card">
      ...
      <van-cell-group inset>
        ...
        <van-cell title="屏幕分辨率" :value="fingerprint.features.screenResolution" />
        <van-cell title="色彩深度" :value="fingerprint.features.screenColorDepth + ' bit'" />
        ...
        <van-cell title="触摸支持" :value="fingerprint.features.touchSupport ? '是' : '否'" />
        <van-cell title="Cookie" :value="fingerprint.features.cookiesEnabled ? '启用' : '禁用'" />
      </van-cell-group>
    </div>
    ...
    <div class="info-card">
      ...
      <van-pull-refresh v-model="refreshing" @refresh="refreshNetworkInfo">
        <van-cell-group inset>
          ...
          />
          <van-cell title="国家" :value="networkInfo.external?.country || '-'" />
          ...
          <van-cell title="时区" :value="networkInfo.external?.timezone || '-'" />
          <van-cell
            title="经纬度"
            :value="networkInfo.external?.latitude && networkInfo.external?.longitude
              ? `${networkInfo.external.latitude}, ${networkInfo.external.longitude}`
              : '-'"
          />
          <van-divider>本地网络</van-divider>
          <van-cell title="在线状态" :value="networkInfo.local?.online ? '在线' : '离线'" />
          <van-cell title="连接类型" :value="networkInfo.local?.connectionType || '-'" />
          <van-cell title="下行速度" :value="networkInfo.local?.downlink ? networkInfo.local.downlink + ' Mbps' : '-'" />
          <van-cell title="延迟" :value="networkInfo.local?.rtt ? networkInfo.local.rtt + ' ms' : '-'" />
          <van-cell title="省流量模式" :value="networkInfo.local?.saveData ? '开启' : '关闭'" />
        </van-cell-group>
      </van-pull-refresh>
    </div>
    ...
    <div class="info-card">
      ...
      <div class="test-request">
        <van-button type="primary" block @click="testRequest" :loading="testing">
          发送测试请求（携带所有头部信息）
        </van-button>
...
const testRequest = async () => {
  ...
  try {
    // 模拟一个请求，实际会携带所有头部信息
    const response = await fetch('http://192.168.0.103:3001/api/v1/test-headers', {
      method: 'GET',
      headers: {
...
const formatRequestHeaders = () => {
  ...
  return `Authorization: Bearer ${localStorage.getItem('finance_token')?.substring(0, 20)}...
