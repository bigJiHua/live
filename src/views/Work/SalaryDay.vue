  ...
  <div class="page-salary-day">
    ...
    >
      ...
      <div class="salary-items">
        ...
        <div class="salary-item">
          <span class="item-label">日均补贴</span>
          <span class="item-value">¥{{ salaryData.formal.dailySubsidy || 0 }}</span>
        </div>
        <div class="salary-item">
    ...
    >
      ...
      <div class="salary-items">
        <div class="salary-item">
          ...
          <span class="item-value">¥{{ item.hourly_wage || item.day_salary / (item.work_hours || 1) }}/小时</span>
        </div>
        <div class="salary-item hours-row">
          <span class="item-label">工作小时</span>
          <van-stepper
            v-model="item.work_hours"
            :min="0"
            :max="24"
            :integer="true"
            :disabled="item.is_calculated"
            @change="calcParttimeIncome(index)"
          />
      ...
      <div class="card-actions">
        <van-button size="small" @click="editParttime(index)">编辑</van-button>
        <van-button
          size="small"
          type="primary"
          :loading="item.saving"
          :disabled="item.is_calculated"
          @click="saveParttime(index)"
        >
          {{ item.is_calculated ? '已保存' : '保存' }}
        </van-button>
      </div>
    </div>
    ...
    <!-- 无工作提示 -->
    <van-empty
      v-if="!salaryData.formal && (!salaryData.parttimes || salaryData.parttimes.length === 0)"
    ...
    <van-popup :show="showEditFormal" position="bottom" round :close-on-click-overlay="false">
      <div class="edit-popup">
        ...
        <van-cell-group inset>
          ...
          <van-field label="日均补贴" :model-value="'¥' + salaryData.formal?.dailySubsidy" disabled readonly />
        ...
        <van-cell-group inset title="手动调整">
          <van-field
            v-model="editFormalData.subsidy"
            label="当日补贴"
            type="number"
            placeholder="覆盖自动计算值"
          >
...
import { getSalaryDay, saveSalaryDay, getJobList } from '@/utils/api/work'
...
// 计算日均补贴（月补贴 / 出勤天数）
const calculateDailySubsidy = (data, type) => {
  if (!data) return 0
  if (type === 'formal') {
    // 从 formalJob 获取月补贴总额和出勤天数
    const job = formalJob.value || {}
    const workDays = job.base_work_days || 22
    const monthSubsidy = parseFloat(job.subsidy_meal || 0) + 
                         parseFloat(job.subsidy_traffic || 0) + 
                         parseFloat(job.subsidy_post || 0)
    return (monthSubsidy / workDays).toFixed(2)
  }
  // 兼职：补贴已经是按天的
  return data.subsidy || 0
}
...
const formalPreviewIncome = computed(() => {
  ...
  // 使用日均补贴
  const subsidy = parseFloat(editFormalData.value.subsidy) || parseFloat(salaryData.value.formal.dailySubsidy) || 0
  ...
  return (base + subsidy - social - fund - tax - cut).toFixed(2)
})
  ...
  try {
    ...
    salaryData.value = {
      formal: data.formal ? { 
        ...
        // 如果补贴是月补贴，需要按天折算
        dailySubsidy: calculateDailySubsidy(data.formal, 'formal'),
      } : null,
...
// 计算兼职收入
const calcParttimeIncome = (index) => {
  const item = salaryData.value.parttimes[index]
  if (!item) return

  const hourly = parseFloat(String(item.hourly_wage || 0))
  const hours = parseFloat(item.work_hours) || 0
  const subsidy = parseFloat(item.subsidy) || 0
  const cut = parseFloat(item.cut) || 0

  item.day_salary = (hourly * hours).toFixed(2)
  item.income = (hourly * hours + subsidy - cut).toFixed(2)
}
...
const saveFormal = async () => {
  ...
  try {
    await saveSalaryDay({
      ...
      work_date: currentDate.value,
      subsidy_meal: parseFloat(editFormalData.value.subsidy) || 0,
...
// 保存兼职（快速保存）
const saveParttime = async (index) => {
  const item = salaryData.value.parttimes[index]
  if (!item || item.saving) return

  // 备用：从 parttimeJobs 查找 job_id
  const jobId = item.job_id || item.id || parttimeJobs.value[index]?.id
  if (!jobId) {
    showToast('无法获取工作信息')
    return
  }
  ...
  try {
    await saveSalaryDay({
      job_id: jobId,
      work_date: currentDate.value,
      work_hours: parseFloat(item.work_hours) || 8,
    })
    showToast('保存成功')
    loadSalaryData()
  } catch (e) {
